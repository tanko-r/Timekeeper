# Backlog

Ideas captured but not yet designed or planned. Pull one into a proper
spec/plan (see `docs/superpowers/`) before implementing.

## Manual Notes from David:

## UI feedback (screenshots)

Captured in-app with Alt+drag. Address the item, then DELETE the
referenced screenshot (see CLAUDE.md).
- [ ] 2026-08-05 14:25 — When editing text on a timer, using the mouse to select actually ends up dragging the timer.  Then the timer seems to land somewhere else.

ALso, timers should visually "make room" for a timer beinng relocated by dragging, similar to Trello. (no screenshot · #/)
- [ ] 2026-08-06 08:42 — There should be a way to multi-select timers, right click for batch menu (including batch delete).  In fact, right click on any timer should open the menu. (no screenshot · #/)
