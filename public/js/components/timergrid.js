...
export function TimerGrid({ settings, onEntryChanged, openEditor }) {
  ...
  // mix every client into one alphabetical run — split it into one headed
  // sub-section per client (same header style "All" uses for its named
  // groups), except a client with only one timer here doesn't earn its own
  // header — those pool into a trailing "Other Clients" sub-section instead
  // (see groupTimersByClient). Deliberately NOT applied to "All": in
  // by-group mode it already renders one headed section per named group
  // (the user's own manual curation — drag order, Sort A–Z — which
  // client-regrouping would fight), and in by-client mode it's already one
  // section per client. Also not applied to a named-group or by-client tab
  // for the same reason. Tried applying it to "All" too at first, but it
  // silently reordered the Ungrouped section, breaking the e2e multi-select
  // test's assumption that freshly-created same-client timers land at the
  // end of Ungrouped in creation order — a real sign it was fighting
  // existing manual order, not just a test-fixture quirk.
  const renderedSections = ACTIVITY[effectiveTab] && tabsEnabled
    ? groupTimersByClient(activityList(effectiveTab))
      .map((g) => ({ key: `${effectiveTab}-${g.key}`, group: null, label: g.label, list: g.list }))
  ...
  return html`
    ...
    ${renderedSections.map((sec) => {
      ...
      // "All" and the activity tabs (Today/Yesterday/Week/Recent, now split
      // into per-client sub-sections — see groupTimersByClient above) show a
      // plain heading (name + count, no rename/delete tools — those live on
      // the active tab above, not per section). A single-section tab (a
      // named group, a specific client) renders just its grid, no heading.
      const showHead = tabsEnabled && (effectiveTab === 'all' || !!ACTIVITY[effectiveTab]);
      return html`
